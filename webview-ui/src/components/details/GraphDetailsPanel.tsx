...
export function GraphDetailsPanel({ state }: { state: State }): React.JSX.Element | null {
  const selection = state.selectedGraphEntity; if (selection === undefined) return null;
  if (selection.kind === 'agent') {
    const agent = state.visualization?.agents[selection.id]; if (agent === undefined) return null;
